...
import { Shield, Zap, Globe, Award, ArrowRight, Star, ExternalLink, Play } from "lucide-react"
      ...
      <div className="relative z-10">
        ...
        <section className="pt-20 pb-16 px-6">
          <div className="text-center max-w-5xl mx-auto">
            ...
            <h1 className="text-5xl md:text-8xl font-extrabold text-white mb-8 tracking-tight leading-[1.05] font-[family-name:var(--font-inter)]">
            ...
            <p className="text-lg md:text-xl text-white/50 max-w-3xl mx-auto mb-12 leading-relaxed font-[family-name:var(--font-source-sans)]">
              We empower industry leaders to build trust and accelerate conversions with our state-of-the-art testimonial infrastructure.
            </p>
        ...
        <section className="py-20 px-6 max-w-7xl mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8">
            {[
              ...
              <div key={index} className="group p-8 rounded-[2.5rem] bg-white/[0.02] border border-white/5 backdrop-blur-3xl hover:bg-white/[0.05] transition-all duration-500">
                <div className={cn("w-12 h-12 rounded-2xl mb-6 flex items-center justify-center", "bg-white/5 border border-white/10 transition-transform group-hover:scale-110")}>
          ...
          <div className="max-w-7xl mx-auto mb-16 text-center md:text-left flex flex-col md:flex-row md:items-end justify-between gap-8">
            ...
            <div className="flex items-center gap-4 p-4 rounded-2xl bg-emerald-500/5 border border-emerald-500/10">
              <div className="flex -space-x-3">
                {[1,2,3,4].map(i => <div key={i} className="w-10 h-10 rounded-full border-2 border-neutral-950 bg-neutral-800" />)}
              </div>
            ...
            <div className="flex flex-wrap items-center justify-center gap-12 md:gap-20 opacity-40 grayscale hover:grayscale-0 transition-all duration-700">
              {['Microsoft', 'Google', 'Amazon', 'Vercel', 'Stripe', 'Salesforce'].map((logo) => (
                <div key={logo} className="h-8 w-32 relative">
                  <div className="flex items-center justify-center h-full text-white font-bold text-xl tracking-tighter italic opacity-60 hover:opacity-100 transition-opacity cursor-default">{logo}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="py-32 px-6">
          <div className="max-w-5xl mx-auto p-12 md:p-20 rounded-[3rem] bg-gradient-to-br from-emerald-500/20 via-blue-500/10 to-transparent border border-white/10 relative overflow-hidden">
            ...
            <div className="relative z-10 text-center">
              <h2 className="text-4xl md:text-6xl font-bold text-white mb-8 tracking-tight font-[family-name:var(--font-inter)]">Ready to transform your business?</h2>
              <div className="flex flex-wrap items-center justify-center gap-6">
                <button className="px-10 py-5 rounded-2xl bg-white text-black font-bold text-lg hover:scale-105 transition-transform shadow-[0_20px_40px_-10px_rgba(255,255,255,0.3)]">Get Started for Free</button>
                <button className="px-10 py-5 rounded-2xl bg-white/5 border border-white/10 text-white font-bold text-lg hover:bg-white/10 transition-colors">Contact Sales</button>
              </div>
            </div>
          </div>
        </section>
            ...
            {[
              ...
              <div key={col.title}>
                ...
                <ul className="space-y-4">
                  {col.links.map(link => <li key={link}><a href="#" className="text-white/40 text-sm hover:text-emerald-400 transition-colors">{link}</a></li>)}
                </ul>
              </div>
            ))}
          </div>
